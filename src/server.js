...
// Only WAY exposes a true exact-lot URL ("Open lot"); SpotHero's link is the event
// map and ParkWhiz's is the event/venue page (their per-lot URLs are absent/404),
// so both are labelled event-level ("View event") — otherwise the button wrongly
// reads "Open lot" for a whole-event page. The event_url fallback only fires when
